...
// Available voices for Puter TTS
const VOICES = [
  { id: "Joanna", label: "Joanna", accent: "US" },
  { id: "Matthew", label: "Matthew", accent: "US" },
...
export function TTSPlayer({ text, level = "B1", className = "" }: TTSPlayerProps) {
  ...
  const generateAndPlay = useCallback(async () => {
    if (!text || isLoading || !window.puter) {
      if (!window.puter) {
        setError("TTS service not ready. Please wait...");
      }
      return;
    }
    ...
    try {
      ...
      // Use first 2000 characters for reasonable length
      const audioText = text.slice(0, 2000);

      // Generate audio using Puter TTS
      const audio = await window.puter.ai.txt2speech(audioText, {
        voice: selectedVoice,
        engine: "neural",
        language: "en-US",
      });
